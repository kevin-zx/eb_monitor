...
def fund2(x):
    return 60.63*np.exp(-0.0007853*x)+294.2*np.exp(-0.01213*x)
...
def db_curve_fit(keyword,count):
    ...
    avgs = np.average(d1)
    scale = avgs/647.8272
    result = {"keyword": keyword, "a": 60.63, "b": -0.0007853, "c": 294.2, "d": -0.01213, "scale": scale,
              "total_avg_sales": total_avg_sales,"count": count}
    st.save(table="fit_args_t", data=result)
