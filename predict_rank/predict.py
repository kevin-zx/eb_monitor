...
class Prediction:
    ...
    def pre(self, keyword, month_sales):
        ...
        # plt.show()
        if len(final_fit) == 4:
            return fund(month_sales, final_fit[0], final_fit[1], final_fit[2], final_fit[3])
        else:
            return fund(month_sales, final_fit[0], final_fit[1], final_fit[2], final_fit[3], final_fit[4])
...
def fit(title, monthsales, data_nid, category):
    ...
    real = db.query(
        "select keyword,AVG(rank),count(1) from eb_result_taobao LEFT JOIN keyword_task ON keyword_id = keyword_task.id where nid = '{0}' AND device = '2' GROUP BY keyword_id;".format(
            data_nid))
